...
class Filter():
    ...
    def filter_vehicle_follow_scenario(dataframe, 
        ...
        car_following_meta = {'dataset_id':[], 'ego_id': [], 'preceding_id': [], 
        ...
        for dataset_id in dataset_ids:
            _df = dataframe[dataframe['dataset_id'] == dataset_id]
            

            fActor = Filter._filter_actors(_df, 
                                        'class', ego_type,
                                        'nLane', 1)
            pActor = Filter._filter_actors(_df,
                                        'class', preceding_type,
                                        'nLane', 1)

            for actor in fActor:
                actor_df = _df[_df['id'] == actor]
                ...
                for p_id in filtered_preceding_id:
                    ...
                    preceding_agent_df = _df[_df['id'] == p_id]
                    ...
                    ego_tracks = _df[(_df['id'] == actor) & (_df['frame'].isin(frames_togather['frame']))]
                    preceding_tracks = _df[(_df['id'] == p_id) & (_df['frame'].isin(frames_togather['frame']))]
                    ...
                    car_following_meta['dataset_id'].append(dataset_id)
